    ...
    def straight_finder(hand):
        ...
        if len(hand)<4:
            ...
            for i in range(0, len(hand)-4):
                if hand[i]+1 == hand[i+1] and hand[i+2]+1==hand[i+3] and hand[i+1]+1 == hand[i+2]:
                    core.cards_on_table.append([hand[i], hand[i+1], hand[i+2], hand[i+3]])
                    return [hand[i], hand[i+1], hand[i+2], hand[i+3]]
    straight = straight_finder(copy)
    while straight is not None:
        for s in straight:
            hand.remove(s)
        new_copy = hand[:]
...
class player:
    ...
    def laydown_check(self, core):
        ...
        if check.count("yes") == 2:
            print("Yippee")
            self.hand = lay_cards_down(self.hand)
            self.laydown = True
        else:
            self.laydown = False
